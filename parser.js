...
TrueTypeFont.prototype = {
    ...
    drawGlyph: function(index, ctx, text_area) {
        ...
        var str1 = JSON.stringify(glyph, null, 4);
        text_area.value = str1;
    ...
    drawMyGlyph: function(index, ctx, ctx2, text_area) {
        ...
        while (p < glyph.points.length) {
            var point = glyph.points[p];
            if ( first === 1 ) {
                ctx.fillStyle = rgbToHex(255,g,b); //red
                ctx.fillRect(point.x, point.y, 30, 30);
                ctx2.moveTo(point.x, point.y);
                first = 0;
            } else {
                ctx.fillStyle = rgbToHex(r,r,r);
...
function ShowTtfFile(arrayBuffer)
{
    ...
    for( var i = 0; i < font.length; i++ ) {
        ...
        if (font.drawMyGlyph(i, ctx, text_area)) {
            ctx.fill();
...
function ShowTtfFile2(arrayBuffer)
{
    var font = new TrueTypeFont(arrayBuffer);

    var width = font.xMax - font.xMin;
    var height = font.yMax - font.yMin;
    var scale = 128 / font.unitsPerEm;
    ...
    for( var i = 0; i < font.length; i++ ) {
        ...
        ctx2.translate(-font.xMin, -font.yMin - height);
        ...
        if (font.drawMyGlyph(i, ctx, ctx2, text_area)) {
            ctx.fill();
            ctx2.fill();
            container.appendChild(canvas);
